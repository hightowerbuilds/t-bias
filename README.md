...
- **Hyperlinks** — OSC 8 links are rendered with underlines and hover-highlighting, and `https?://` URLs are auto-detected via regex; clicking opens them via the Tauri shell plugin
...
**Frontend** (TypeScript + SolidJS) — terminal core in [src/terminal/](src/terminal/):
...
- `Renderer.ts` / `IRenderer.ts` — Canvas 2D renderer with three-pass drawing (backgrounds, glyphs, decorations), glyph atlas caching, and sub-row dirty column tracking; `IRenderer` is the renderer interface
- `GlyphAtlas.ts` — OffscreenCanvas-backed glyph cache (multiple pages, LRU eviction)
- `TerminalCore.ts` — wires Parser + Screen + VirtualCanvas together behind a single API
- `TerminalHost.ts` — DOM orchestrator: 3-layer canvas stack, input handling, cursor blink, write coalescing, URL hover/click, debug overlay
- `Selection.ts` — click-drag text selection and clipboard copy
- `input.ts` — keyboard event to VT escape sequence mapping

**Top-level UI** (SolidJS) in [src/](src/):
- `App.tsx` — tab store, tab bar, persistence, top-level layout
- `Terminal.tsx` — terminal tab view, wraps `TerminalHost`
- `Panes.tsx` / `pane-tree.ts` — recursive split-pane tree with draggable dividers
- `FileExplorer.tsx` — file-explorer tab (directory tree UI)
- `Editor.tsx` / `editor/` — code-editor tab (`EditorHost`, `EditorBuffer`, `Tokenizer`)
- `FlipExplorer.tsx` — flip-view explorer overlay

**Backend** (Rust) in [src-tauri/src/](src-tauri/src/):
...
- `fs_ops.rs` — filesystem operations exposed via Tauri commands for the file explorer and editor tabs
- `lib.rs` / `main.rs` — Tauri setup, plugin registration, command wiring
...
```toml
shell = ""                      # empty uses your system default shell (e.g. "/bin/zsh")
...
58 tests via Vitest covering cursor movement, text output, line wrapping, erase operations, SGR (truecolor, palette, colon sub-params), scroll regions, tab stops, DEC graphics charset, alternate screen, terminal modes, device responses, soft reset, insert/delete operations, and typed-array consistency across TUI-like scenarios.
