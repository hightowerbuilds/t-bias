//#region node_modules/@chenglou/pretext/dist/generated/bidi-data.js
var latin1BidiTypes = [
	"BN",
	"BN",
	"BN",
...
function computeBidiLevels(str) {
	...
	for (let i = 0; i < len;) {
		...
		for (let j = 0; j < codeUnitLength; j++) types[i + j] = t;
		i += codeUnitLength;
	}
...
function isCJK(s) {
	for (let i = 0; i < s.length; i++) {
		...
		if (first >= 55296 && first <= 56319 && i + 1 < s.length) {
			...
			if (second >= 56320 && second <= 57343) {
				if (isCJKCodePoint((first - 55296 << 10) + (second - 56320) + 65536)) return true;
				i++;
				continue;
			}
		}
...
function joinReversedPrefixParts(prefixParts, tail) {
	const parts = [];
	for (let i = prefixParts.length - 1; i >= 0; i--) parts.push(prefixParts[i]);
	parts.push(tail);
	return joinTextParts(parts);
}
function splitSegmentByBreakKind(segment, isWordLike, start, whiteSpaceProfile) {
	if (!breakCharRe.test(segment)) return [{
		text: segment,
		isWordLike,
		kind: "text",
		start
	...
	for (const s of wordSegmenter.segment(normalized)) for (const piece of splitSegmentByBreakKind(s.segment, s.isWordLike ?? false, s.index, whiteSpaceProfile)) {
		...
		function appendPieceToPrevious() {
			if (mergedSingleCharRunChars[prevIndex] !== null) {
				mergedTextParts[prevIndex] = [materializeDeferredSingleCharRun(mergedTexts, mergedSingleCharRunChars, mergedSingleCharRunLengths, prevIndex)];
		...
		if (profile.carryCJKAfterClosingQuote && isText && mergedLen > 0 && mergedKinds[prevIndex] === "text" && pieceContainsCJK && mergedContainsCJK[prevIndex] && mergedEndsWithClosingQuote[prevIndex]) appendPieceToPrevious();
		else if (isText && mergedLen > 0 && mergedKinds[prevIndex] === "text" && isCJKLineStartProhibitedSegment(piece.text) && mergedContainsCJK[prevIndex]) appendPieceToPrevious();
		else if (isText && mergedLen > 0 && mergedKinds[prevIndex] === "text" && mergedEndsWithMyanmarMedialGlue[prevIndex]) appendPieceToPrevious();
		...
		} else if (repeatableSingleCharRunChar !== null && mergedLen > 0 && mergedKinds[prevIndex] === "text" && mergedSingleCharRunChars[prevIndex] === repeatableSingleCharRunChar) mergedSingleCharRunLengths[prevIndex] = (mergedSingleCharRunLengths[prevIndex] ?? 1) + 1;
		else if (isText && !piece.isWordLike && mergedLen > 0 && mergedKinds[prevIndex] === "text" && (isLeftStickyPunctuationSegment(piece.text) || piece.text === "-" && mergedWordLike[prevIndex])) appendPieceToPrevious();
		else {
...
function getSegmentBreakableFitAdvances(seg, metrics, cache, emojiCorrection, mode) {
	...
	const graphemeSegmenter = getSharedGraphemeSegmenter$1();
	const graphemes = [];
	for (const gs of graphemeSegmenter.segment(seg)) graphemes.push(gs.segment);
	...
	if (mode === "pair-context" || graphemes.length > MAX_PREFIX_FIT_GRAPHEMES) {
		...
		let previousWidth = 0;
		for (const grapheme of graphemes) {
			const currentWidth = getCorrectedSegmentWidth(grapheme, getSegmentMetrics(grapheme, cache), emojiCorrection);
			if (previousGrapheme === null) advances.push(currentWidth);
			else {
...
function fitSoftHyphenBreak(graphemeFitAdvances, initialWidth, maxWidth, lineFitEpsilon, discretionaryHyphenWidth) {
	...
	while (fitCount < graphemeFitAdvances.length) {
		...
		if ((fitCount + 1 < graphemeFitAdvances.length ? nextWidth + discretionaryHyphenWidth : nextWidth) > maxWidth + lineFitEpsilon) break;
...
function normalizeLineStartChunkIndexFromHint(prepared, chunkIndex, cursor) {
	...
	while (nextChunkIndex < prepared.chunks.length && cursor.segmentIndex >= prepared.chunks[nextChunkIndex].consumedEndSegmentIndex) nextChunkIndex++;
...
function countPreparedLines(prepared, maxWidth) {
	if (prepared.simpleLineWalkFastPath) return walkPreparedLinesSimple(prepared, maxWidth);
	return walkPreparedLines(prepared, maxWidth);
}
function walkPreparedLinesSimple(prepared, maxWidth, onLine) {
	const { widths, kinds, breakableFitAdvances } = prepared;
	if (widths.length === 0) return 0;
	const fitLimit = maxWidth + getEngineProfile().lineFitEpsilon;
	let lineCount = 0;
	let lineW = 0;
	...
	function appendBreakableSegmentFrom(segmentIndex, startGraphemeIndex) {
		...
		for (let g = startGraphemeIndex; g < fitAdvances.length; g++) {
			...
			if (!hasContent) startLineAtGrapheme(segmentIndex, g, gw);
	...
	while (i < widths.length) {
		...
		if (!hasContent) {
			if (w > maxWidth && breakableFitAdvances[i] !== null) appendBreakableSegmentFrom(i, 0);
...
function stepPreparedChunkLineGeometry(prepared, cursor, chunkIndex, maxWidth) {
	...
	function appendBreakableSegmentFrom(segmentIndex, startGraphemeIndex) {
		...
		for (let g = startGraphemeIndex; g < fitAdvances.length; g++) {
			...
			if (!hasContent) startLineAtGrapheme(segmentIndex, g, gw);
				...
				if (lineW + gw > fitLimit) return finishLine();
				lineW += gw;
				lineEndSegmentIndex = segmentIndex;
				lineEndGraphemeIndex = g + 1;
			}
	...
	function maybeFinishAtSoftHyphen(segmentIndex) {
		...
		if (pendingBreakFitWidth <= fitLimit) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
		return null;
	}
	for (let i = cursor.segmentIndex; i < chunk.endSegmentIndex; i++) {
		...
		if (lineW + w > fitLimit) {
			const currentBreakFitWidth = lineW + (kind === "tab" ? 0 : lineEndFitAdvances[i]);
			const currentBreakPaintWidth = lineW + (kind === "tab" ? w : lineEndPaintAdvances[i]);
			if (pendingBreakKind === "soft-hyphen" && engineProfile.preferEarlySoftHyphenBreak && pendingBreakFitWidth <= fitLimit) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
...
function layoutNextLineRange$1(prepared, start, maxWidth) {
	const end = {
...
function buildBaseCjkUnits(segText, engineProfile) {
	...
	function appendToUnit(grapheme, graphemeContainsCJK) {
		...
		if (grapheme.length === 1 && leftStickyPunctuation.has(grapheme)) unitEndsWithClosingQuote = unitEndsWithClosingQuote || graphemeEndsWithClosingQuote;
		else unitEndsWithClosingQuote = graphemeEndsWithClosingQuote;
		unitIsSingleKinsokuEnd = false;
	}
...
function measureAnalysis(analysis, font, includeSegments, wordBreak) {
	...
	function pushMeasuredTextSegment(text, kind, start, wordLike, allowOverflowBreaks) {
		...
		if (allowOverflowBreaks && wordLike && text.length > 1) {
			...
			if (isNumericRunSegment(text)) fitMode = "pair-context";
			else if (engineProfile.preferPrefixWidthsForBreakableRuns) fitMode = "segment-prefixes";
			pushMeasuredSegment(text, width, lineEndFitAdvance, lineEndPaintAdvance, kind, start, getSegmentBreakableFitAdvances(text, textMetrics, cache, emojiCorrection, fitMode));
			return;
		}
...
function prepareInternal(text, font, includeSegments, options) {
	...
	return measureAnalysis(analyzeText(text, getEngineProfile(), options?.whiteSpace, wordBreak), font, includeSegments, wordBreak);
}
...
function layoutNextLineRange(prepared, start, maxWidth) {
	const line = layoutNextLineRange$1(prepared, start, maxWidth);
	if (line === null) return null;
...
//#endregion
export { clearCache, layout, layoutNextLine, layoutNextLineRange, layoutWithLines, materializeLineRange, measureLineStats, measureNaturalWidth, prepare, prepareWithSegments, setLocale, walkLineRanges };
