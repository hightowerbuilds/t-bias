...
export class Parser {
  ...
  private flushPrintBuffer() {
    ...
    // ASCII fast path: skip Intl.Segmenter for pure ASCII output (covers ~95%
    // of typical terminal content — each ASCII char is its own grapheme cluster)
    let allAscii = true;
    for (let i = 0; i < buf.length; i++) {
      if (buf.charCodeAt(i) > 0x7e) { allAscii = false; break; }
    }
    if (allAscii) {
      for (let i = 0; i < buf.length; i++) this.handler.print(buf[i]);
      return;
    }
