...
describe("TUI typed array consistency", () => {
  ...
  it("50 scrolls produce no row aliasing or duplicates", () => {
    const c = term(20, 5);
    // Write 55 lines — the first 50 scroll off the top
    for (let i = 0; i < 55; i++) {
      if (i > 0) c.write("\r\n");
      c.write(`line${i}`);
    }
    // Screen should show lines 50-54
    expect(vcTextRow(c, 0)).toBe("line50");
    expect(vcTextRow(c, 1)).toBe("line51");
    expect(vcTextRow(c, 2)).toBe("line52");
    expect(vcTextRow(c, 3)).toBe("line53");
    expect(vcTextRow(c, 4)).toBe("line54");
    // Verify no two logical rows point to the same physical row
    const vc = c.virtualCanvas;
    const offsets = new Set<number>();
    for (let r = 0; r < 5; r++) {
      const off = vc.rowOffset(r);
      expect(offsets.has(off)).toBe(false);
      offsets.add(off);
    }
  });

  it("DECSTBM partial scroll does not corrupt rows outside the region", () => {
    const c = term(20, 5);
    // Fill all rows
    c.write("head\r\nA\r\nB\r\nC\r\nfoot");
    expect(vcTextRow(c, 0)).toBe("head");
    expect(vcTextRow(c, 4)).toBe("foot");
    // Set scroll region to rows 2-4 (1-based), leaving rows 0 and 4 fixed
    c.write("\x1b[2;4r");
    // Move to bottom of scroll region and write to force scroll
    c.write("\x1b[4;1H");    // row 4 in 1-based = row 3 (0-based)
    c.write("\r\nD\r\nE");   // two scrolls within region [1..3]
    // Rows outside region should be unchanged
    expect(vcTextRow(c, 0)).toBe("head");
    expect(vcTextRow(c, 4)).toBe("foot");
    // Region should have scrolled: A,B,C → C,D,E
    expect(vcTextRow(c, 1)).toBe("C");
    expect(vcTextRow(c, 2)).toBe("D");
    expect(vcTextRow(c, 3)).toBe("E");
  });

  it("resize after scrolling preserves correct row order", () => {
    const c = term(20, 5);
    // Fill and scroll
    for (let i = 0; i < 10; i++) {
      if (i > 0) c.write("\r\n");
    ...
    // Screen: row5..row9
    expect(vcTextRow(c, 0)).toBe("row5");
    expect(vcTextRow(c, 4)).toBe("row9");
    // Resize (stays within physCols/physRows, no reallocation)
    c.resize(20, 5);
    // Content should still be correct after resize resets rowMap
    expect(vcTextRow(c, 0)).toBe("row5");
    expect(vcTextRow(c, 1)).toBe("row6");
    expect(vcTextRow(c, 2)).toBe("row7");
    expect(vcTextRow(c, 3)).toBe("row8");
    expect(vcTextRow(c, 4)).toBe("row9");
  });
});
