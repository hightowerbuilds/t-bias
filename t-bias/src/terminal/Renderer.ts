...
export class CanvasRenderer implements IRenderer {
  ...
  draw(state: RenderState) {
    ...
    for (let row = 0; row < rows; row++) {
      if (!fullRedraw && !bitmap[row]) continue;
      const y = row * cellHeight;

      // Sub-row dirty column range — only redraw the changed columns
      const colStart = fullRedraw ? 0 : state.dirtyColStart[row];
      const colEnd = fullRedraw ? cols : state.dirtyColEnd[row];
